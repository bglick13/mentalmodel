...
class DocsSiteTest(unittest.TestCase):
    ...
    def test_phase_26_docs_cover_dashboard_ui(self) -> None:
        ...
        self.assertIn("Hosted mode", cli_ui)
        self.assertIn("Local mode", cli_ui)
    ...
    def test_remote_docs_cover_repo_linked_phase_one(self) -> None:
        ...
        overview_page = (repo_root / "cli" / "overview.mdx").read_text(encoding="utf-8")
        ...
        self.assertIn("mentalmodel remote publish-catalog", remote_page)
        self.assertIn("mentalmodel remote doctor", remote_page)
        self.assertIn("runtime.completed_run_upload", remote_page)
        self.assertIn("manual recovery", remote_page)
        self.assertIn("repo-owned", remote_page)
        self.assertIn("remote catalog snapshots", remote_page)
        self.assertIn("Live execution flow", remote_page)
        self.assertIn("Choosing a mode", remote_page)
        self.assertIn("hosted repo-linked mode", remote_page)
        self.assertIn("local workspace mode", remote_page)
        self.assertIn("persisted run view", remote_page)
        self.assertIn("mentalmodel remote link", readme)
        self.assertIn("mentalmodel remote status", readme)
        self.assertIn("mentalmodel remote publish-catalog", readme)
        self.assertIn("uploads the completed run bundle automatically", readme)
        self.assertIn("streams live execution state", readme)
