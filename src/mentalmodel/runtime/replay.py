...
class RunDiff:
    ...
    state_equal: bool
    state_run_a: dict[str, JsonValue]
    state_run_b: dict[str, JsonValue]
    ...
    def as_dict(self) -> dict[str, JsonValue]:
        return {
            ...
            "state_run_a": self.state_run_a,
            "state_run_b": self.state_run_b,
...
def build_replay_report(
    *,
    ...
    return ReplayReport(
        ...
        output_node_ids=tuple(sorted(outputs.keys())),
        state_node_ids=tuple(sorted(state.keys())),
    )
...
def build_run_diff(
    *,
    ...
    outputs_a = _load_mapping_payload(
        runs_dir=runs_dir,
        graph_id=summary_a.graph_id,
        run_id=summary_a.run_id,
        filename="outputs.json",
    ...
    outputs_b = _load_mapping_payload(
        runs_dir=runs_dir,
        graph_id=summary_b.graph_id,
        run_id=summary_b.run_id,
        filename="outputs.json",
...
def diff_nodes(
    *,
    ...
    outputs_run_a: Mapping[str, JsonValue],
    outputs_run_b: Mapping[str, JsonValue],
    node_id: str | None = None,
) -> tuple[NodeDiff, ...]:
    inputs_run_a = _latest_inputs_by_node(records_run_a)
    inputs_run_b = _latest_inputs_by_node(records_run_b)
    records_by_node_run_a = _records_by_node(records_run_a)
    records_by_node_run_b = _records_by_node(records_run_b)
    ...
    if node_id is not None:
        ...
        node_ids = tuple(
            sorted(
                set(records_by_node_run_a)
                | set(records_by_node_run_b)
                | set(inputs_run_a)
                | set(inputs_run_b)
                | set(outputs_run_a)
                | set(outputs_run_b)
    ...
    for current_node_id in node_ids:
        records_a = records_by_node_run_a.get(current_node_id, tuple())
        records_b = records_by_node_run_b.get(current_node_id, tuple())
        input_a = inputs_run_a.get(current_node_id)
        input_b = inputs_run_b.get(current_node_id)
        output_a = outputs_run_a.get(current_node_id)
        output_b = outputs_run_b.get(current_node_id)
        node_diffs.append(
            NodeDiff(
...
def diff_invariants(
    *,
    ...
    diffs = [
        ...
        for current_node_id in sorted(invariant_nodes)
    ]
    return tuple(diffs)
...
def _records_by_node(
    ...
    for record in records:
        grouped[_require_str(record, "node_id")].append(record)
    return {
        node_id: tuple(sorted(node_records, key=lambda record: _require_int(record, "sequence")))
...
def _latest_inputs_by_node(
    records: tuple[dict[str, JsonValue], ...],
) -> dict[str, JsonValue]:
    latest: dict[str, tuple[int, JsonValue]] = {}
    for record in records:
        ...
        if current is None or sequence > current[0]:
            latest[node_id] = (sequence, payload["inputs"])
    return {node_id: value for node_id, (_, value) in latest.items()}
...
def _invariant_outcomes(
    ...
    for record in records:
        ...
        node_id = _require_str(record, "node_id")
        sequence = _require_int(record, "sequence")
        current = outcomes.get(node_id)
        if current is None or sequence > current[0]:
            outcomes[node_id] = (sequence, passed)
...
def _load_mapping_payload(
    *,
    ...
    if not isinstance(raw, dict):
        raise RunInspectionError(
            f"Run artifact {filename!r} does not contain a JSON object for {key!r}."
        )
    return raw
